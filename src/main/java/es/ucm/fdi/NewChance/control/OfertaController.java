...
public class OfertaController {
    ...
    public String pujar(
        ...
        BigDecimal saldoSiPuja = saldo.subtract(precio.add(precio.multiply(porcentaje)
                                ...
                                .add(envio)); //calculamos el saldo del usuario si este realizase la puja que está intentando realizar
    ...
    @Transactional 
    public String listaPujas(@PathVariable long id, Model model, HttpSession session) {  
        
        Producto prod = entityManager.find(Producto.class, id);
        
        List<Oferta> pujas = entityManager.createNamedQuery("Oferta.pujas")
    ...
    @Transactional 
    public String listaPrecios(@PathVariable long id, Model model, HttpSession session) {    

        Producto prod = entityManager.find(Producto.class, id);
        List<Oferta> precios = entityManager.createNamedQuery("Oferta.precios")
                                .setParameter("productoId", id)
                                .getResultList();
    ...
    @Transactional
    public String listaVentas(@PathVariable long id, Model model, HttpSession session) {  
        
        Producto prod = entityManager.find(Producto.class, id);

        
        List<Oferta> ventas = entityManager.createNamedQuery("Oferta.transaction")
                                .setParameter("productoId", id)
                                .getResultList();
