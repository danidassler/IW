...
public class MensajeController {
	...
	public String getMessages(@PathVariable long id, Model model, HttpSession session) {
		...
		try {
			adminId = (long)entityManager.createNamedQuery("Mensaje.findAdmin")
	...
	public List<Mensaje.Transfer> messagesFromAdmin(@PathVariable long adminId, 
	HttpSession session) {
	...
	public List<Mensaje.Transfer> messagesFromClient(@PathVariable long clientId, HttpSession session) {
		...
		if(clientId != 0){
			...
			List<Mensaje> mensajesPending = entityManager.createNamedQuery("Mensaje.findNullMsg")
				.getResultList();
	...
	public List<Mensaje.Transfer> getAllMessages(long receptorId, HttpSession session){
		...
		List<Mensaje> mensajesRecibidos = entityManager.createNamedQuery("Mensaje.recibidos")
			.setParameter("userId", userId)
			.setParameter("clienteId", receptorId)
			.getResultList();

		List<Mensaje> mensajesEnviados = entityManager.createNamedQuery("Mensaje.enviados")
			.setParameter("userId", userId)
			.setParameter("clienteId", receptorId)
			.getResultList();
